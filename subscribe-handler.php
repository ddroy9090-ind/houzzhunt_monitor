...
$country = trim((string)($_POST['country1'] ?? ''));
$token   = (string)($_POST['g-recaptcha-response'] ?? '');

$_SESSION['subscribe_form_old'] = [
    'name'     => $name,
    'email'    => $email,
    'phone'    => $phone,
    'country1' => $country,
...
$redirectWithError = static function (string $message) use ($redirectTarget): void {
    $_SESSION['subscribe_form_error'] = $message;
...
if ($secretKey === '' || $secretKey === 'your-secret-key-here') {
    error_log('Subscribe form: reCAPTCHA secret key is not configured.');
...
try {
    $pdo = hh_db();
    $stmt = $pdo->prepare(
        'INSERT INTO contact_form (name, email, phone, country, ip_address, user_agent, created_at)
    ...
    error_log('Subscribe form: database error - ' . $exception->getMessage());
    $redirectWithError('We were unable to submit your request. Please try again later.');
}

unset($_SESSION['subscribe_form_old']);
